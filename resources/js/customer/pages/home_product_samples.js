...
async function loadProductSamples() {
  ...
  try {
    // Show loading state
    container.innerHTML = '<p style="text-align: center; color: #999; padding: 40px;">Loading product samples...</p>';
    ...
    if (!samples || samples.length === 0) {
      container.innerHTML = '<p style="text-align: center; color: #999; padding: 40px;">No product samples available yet.</p>';
      return;
    }
    ...
    samples.forEach((sample, index) => {
      ...
      const imageUrl = sample.images && sample.images.length > 0
    ...
    container.innerHTML = '<p style="text-align: center; color: #999; padding: 40px;">Unable to load product samples. Please try again later.</p>';
  }
}
