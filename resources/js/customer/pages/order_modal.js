...
export async function openOrderModal(productId) {
  try {
    ...
    optionsContainer.innerHTML = '<p style="text-align: center; color: #999;">Loading order configuration...</p>';
...
function selectOption(optionId, optionTypeId, btnElement) {
  ...
  const siblings = btnElement.parentElement.querySelectorAll('.order_modal_option_type_btn');
...
function renderRushFees() {
  ...
  templateData.rush_fees.forEach((rushFee) => {
    ...
    const timeframePercentage = mainTimeframe?.percentage || rushFee.percentage_increase || 0;
...
function calculateOrderPrice() {
  ...
  const pricingEntry = template.pricings.find((p) => p.combination_key === combinationKey);
  if (!pricingEntry) {
  ...
  const applicableDiscount = template.discounts.find((d) => d.min_quantity <= quantity);
  ...
  if (selectedRushFeeId) {
    const rushFee = templateData.rush_fees.find((rf) => rf.id == selectedRushFeeId);
  ...
  const totalPrice = basePrice - discountAmount + layoutFeeAmount + rushFeeAmount;
...
function validateForm() {
  ...
  if (!templateData || !templateData.template) {
    showFormMessage('Form data lost. Please refresh and try again.', 'error');
    return false;
  }
  ...
  if (quantity < (template.min_order || 1)) {
    showFormMessage(`Minimum quantity is ${template.min_order || 1}`, 'error');
    return false;
  }
...
async function handleFormSubmit(e) {
  ...
  try {
    ...
    if (!templateData || !templateData.template) {
      throw new Error('Form data lost (templateData is null). Please refresh the page and try again.');
    }
