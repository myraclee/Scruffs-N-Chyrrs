...
function initializeProductDetail() {
  try {
    ...
    gallery.innerHTML = '<div class="price_image_error_text">Unable to load product information.</div>';
  }
}
...
async function loadAndRenderPriceImages() {
  if (!priceImages || priceImages.length === 0) {
    gallery.innerHTML = '<div class="price_image_error_text">No price images available for this product.</div>';
    return;
  }
  ...
  priceImages.forEach((image, index) => {
    ...
    img.addEventListener('error', () => {
      ...
      wrapper.innerHTML = '<div class="price_image_error_text">Image failed to load</div>';
    });
...
function handleImageError(wrapper) {
  wrapper.classList.add('error');
  wrapper.innerHTML = '<div class="price_image_error_text">Failed to load image</div>';
}
