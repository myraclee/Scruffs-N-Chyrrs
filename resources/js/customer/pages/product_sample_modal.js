...
function initializeModal() {
    ...
    fullscreenBtn = modalOverlay?.querySelector(
        ".sample_gallery_fullscreen_btn",
    );
    mainImage = modalOverlay?.querySelector(".sample_gallery_main_image");
    mainImageWrapper = modalOverlay?.querySelector(
        ".sample_gallery_main_image_wrapper",
    );
    thumbnailsGrid = modalOverlay?.querySelector(
        ".sample_gallery_thumbnails_grid",
    );
    ...
    prevBtn = document.getElementById("sampleGalleryPrev");
    nextBtn = document.getElementById("sampleGalleryNext");
    ...
    prevBtn?.addEventListener("click", () => {
        if (currentSample && currentImageIndex > 0) {
    ...
    nextBtn?.addEventListener("click", () => {
        if (
            currentSample &&
            currentImageIndex < currentSample.images.length - 1
        ) {
            displayMainImage(currentImageIndex + 1);
        }
    });
...
/**
 * Update prev/next arrow disabled states based on current index
 */
...
function updateActiveThumbnail(index) {
    ...
    const thumbnails = thumbnailsGrid.querySelectorAll(
        ".sample_gallery_thumbnail",
    );
