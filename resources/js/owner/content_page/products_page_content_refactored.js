...
async function loadProductsFromAPI() {
  try {
    ...
    products_container.innerHTML = '<p style="color: #999;">Error loading products</p>';
  }
}
...
products_prev_price_image.addEventListener('click', () => {
  ...
  products_current_viewer_index = (products_current_viewer_index - 1 + product.price_images.length) % product.price_images.length;
  updatePriceImageViewer(product.price_images);
});
...
products_next_price_image.addEventListener('click', () => {
  ...
  products_current_viewer_index = (products_current_viewer_index + 1) % product.price_images.length;
  updatePriceImageViewer(product.price_images);
});
