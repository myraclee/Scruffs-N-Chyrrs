...
async function loadMaterialsAndProducts() {
  try {
    ...
    if (emptyInventoryRow) {
      emptyInventoryRow.innerHTML = '<td colspan="3" class="text-center" style="padding: 60px; color: #999;">Error loading inventory</td>';
    }
  }
}
...
async function saveMaterial(mode) {
  ...
  const quantityInputs = isAdd
    ...
    : Array.from(editMaterialModal.querySelectorAll('.edit_quantity_input'));
  ...
  const checkboxes = isAdd
    ...
    : Array.from(editMaterialModal.querySelectorAll('.edit_product_checkbox'));
  ...
  checkboxes.forEach(checkbox => {
    if (checkbox.checked) {
      ...
      const quantityInput = isAdd
        ? addMaterialModal.querySelector(`.add_quantity_input[data-product-id="${productId}"]`)
        : editMaterialModal.querySelector(`.edit_quantity_input[data-product-id="${productId}"]`);
...
async function openEditModal(btn, materialId) {
  ...
  editMaterialModal.querySelectorAll('.edit_product_checkbox').forEach(checkbox => {
    checkbox.checked = false;
    const quantityInput = editMaterialModal.querySelector(`.edit_quantity_input[data-product-id="${checkbox.getAttribute('data-product-id')}"]`);
    if (quantityInput) {
  ...
  if (material.products && material.products.length > 0) {
    material.products.forEach(product => {
      const checkbox = editMaterialModal.querySelector(`.edit_product_checkbox[data-product-id="${product.id}"]`);
      const quantityInput = editMaterialModal.querySelector(`.edit_quantity_input[data-product-id="${product.id}"]`);
...
function updateStatusCards() {
  ...
  if (lowStocks.length > 0) {
    lowStockList.innerHTML = lowStocks.map(name => `<p>${name}</p>`).join('');
  } else {
  ...
  if (outOfStocks.length > 0) {
    outOfStockList.innerHTML = outOfStocks.map(name => `<p>${name}</p>`).join('');
  } else {
...
function resetAddModal() {
  ...
  addMaterialModal.querySelectorAll('.add_product_checkbox').forEach(checkbox => {
    checkbox.checked = false;
  });

  addMaterialModal.querySelectorAll('.add_quantity_input').forEach(input => {
    input.value = '0';
  });
}
