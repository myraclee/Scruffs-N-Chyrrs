...
class FormState {
  ...
  save(formId) {
    ...
    try {
      localStorage.setItem(this.getStorageKey(formId), JSON.stringify(data));
      console.log(`Form "${formId}" auto-saved to localStorage`);
    } catch (error) {
      console.error(`Error saving form "${formId}" to localStorage:`, error);
    }
  }
  ...
  restore(formId) {
    ...
    try {
      ...
      console.error(`Error restoring form "${formId}" from localStorage:`, error);
      return null;
    }
  }
  ...
  enableAutoSave(formId, interval = this.autoSaveInterval) {
    ...
    console.log(`Auto-save enabled for form "${formId}" (interval: ${interval}ms)`);
  }
  ...
  disableAutoSave(formId) {
    ...
    if (autoSaveData) {
      ...
      if (autoSaveData.handlers.saveHandler) {
        form.removeEventListener('input', autoSaveData.handlers.saveHandler);
        form.removeEventListener('change', autoSaveData.handlers.saveHandler);
      }
  ...
  clearAll() {
    try {
      ...
      console.log(`Cleared all saved form data (${keysToRemove.length} forms)`);
    } catch (error) {
  ...
  watchForChanges(formId, onChange) {
    ...
    const changeHandler = () => {
      ...
      const stateChanged = JSON.stringify(initialState) !== JSON.stringify(currentState);
